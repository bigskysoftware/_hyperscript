...
function createPanel(_hyperscript, ttd, timeline, domRestorer, recorder) {
  ...
  function refreshElements() {
    ...
    var sel = _hyperscript.config.attributes.replaceAll(" ", "").split(",").map(function(a) {
      return "[" + a + "]";
    }).join(",");
  ...
  function hsAttrsSelector() {
    return _hyperscript.config.attributes.replaceAll(" ", "").split(",").map(function(a) {
      return "[" + a + "]";
    }).join(",");
  }
...
function debuggerPlugin(_hyperscript) {
...
export {
  debuggerPlugin as default
};
//# sourceMappingURL=debugger.esm.js.map
