...
Components let you define reusable custom elements using a `<script type="text/hyperscript-template">` tag. You write your markup with `${}` interpolation, attach hyperscript with `_=`, and hyperscript handles reactive re-rendering automatically.
...
```html
<script src="https://cdn.jsdelivr.net/npm/hyperscript.org@__VERSION__/dist/_hyperscript.min.js"></script>
<script src="https://cdn.jsdelivr.net/npm/hyperscript.org@__VERSION__/dist/ext/component.min.js"></script>
```
...
To define a component, create a `<script type="text/hyperscript-template">` with a `component` attribute set to your tag name (which must contain a dash, per the custom elements spec). Then use the component anywhere in your page like a normal HTML element.
...
```html
<script type="text/hyperscript-template" component="my-display" _="init set ^val to attrs.initial">
  <span>${^val}</span>
</script>
...
```html
<script type="text/hyperscript-template" component="my-counter" _="init set ^count to 0">
...
```html
<script type="text/hyperscript-template" component="start-at" _="init set ^val to @data-start as Int">
  <span>${^val}</span>
</script>
...
```html
<script type="text/hyperscript-template" component="<tag-name>" [_="<hyperscript>"]>
