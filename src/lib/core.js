...
(function (root, factory) {
	...
	return (function () {
			...
			function makeTokensObject(tokens, consumed, source) {
				...
				function matchTokenType(type1, type2, type3, type4) {
					if (
						currentToken() &&
						currentToken().type &&
						[type1, type2, type3, type4].indexOf(
							currentToken().type
						) >= 0
					) {
						return consumeToken();
				...
				function lastWhitespace() {
					if (
						...
						consumed[consumed.length - 1].type === "WHITESPACE"
					) {
			...
			function tokenize(string, template) {
				...
				while (position < source.length) {
					if (currentChar() === "-" && nextChar() === "-") {
						...
						if (isWhitespace(currentChar())) {
							...
							isAlpha(currentChar()) ||
							(!inTemplate() && isIdentifierChar(currentChar()))
						) {
				...
				function consumeString() {
					...
					if (currentChar() !== startChar) {
						throw Error(
							"Unterminated string at " + positionString(string)
						);
					} else {
		...
		var _parser = (function () {
			...
			function parseElement(type, tokens, root) {
				...
				if (elementDefinition) {
					...
					var parseElement = elementDefinition(
						_parser,
						_runtime,
						tokens,
						root
			...
			function requireElement(type, tokens, message, root) {
				...
				return (
					result ||
					raiseParseError(tokens, message || "Expected " + type)
				);
			}
			...
			function addCommand(keyword, definition) {
				...
				var commandDefinitionWrapper = function (
					parser,
					runtime,
					tokens
				) {
			...
			function addFeature(keyword, definition) {
				...
				var featureDefinitionWrapper = function (
					parser,
					runtime,
					tokens
				) {
			...
			addGrammarElement("feature", function (parser, runtime, tokens) {
				if (tokens.matchOpToken("(")) {
					var featureElement = parser.requireElement(
						"feature",
						tokens
					);
			...
			addGrammarElement("command", function (parser, runtime, tokens) {
				if (tokens.matchOpToken("(")) {
					var commandElement = parser.requireElement(
						"command",
						tokens
					);
					tokens.requireOpToken(")");
				...
				if (commandDefinition) {
					var commandElement = commandDefinition(
						parser,
						runtime,
						tokens
					);
			...
			addGrammarElement(
				...
				function (parser, runtime, tokens, root) {
					if (tokens.matchToken("unless")) {
						root.endToken = tokens.lastMatch();
						var conditional = parser.requireElement(
							"expression",
							tokens
						);
						var unless = {
			...
			function raiseParseError(tokens, message) {
				message =
					(message ||
						"Unexpected Token : " + tokens.currentToken().value) +
					"\n\n" +
			...
			function parseStringTemplate(tokens) {
				...
				do {
					...
					if (tokens.currentToken().value === "$") {
						...
						returnArr[returnArr.length - 1] += token
							? token.value
							: "";
					}
		...
		var _runtime = (function () {
				...
				while (true) {
					...
					if (next == null) {
						...
						next.then(function (resolvedNext) {
							...
							if (
								ctx.meta.errorHandler &&
								!ctx.meta.handlingError
							) {
								ctx.meta.handlingError = true;
					...
					return new Promise(function (resolve, reject) {
						...
						Promise.all(linearized)
							.then(function (values) {
								...
								try {
									var apply = parseElement.op.apply(
										parseElement,
										values
									);
									resolve(apply);
			...
			function parse(src) {
				...
				if (_parser.commandStart(tokens.currentToken())) {
					var commandList = _parser.parseElement(
						"commandList",
						tokens
					);
					...
					var hyperscript = _parser.parseElement(
						"hyperscript",
						tokens
					);
					return hyperscript;
			...
			function evaluate(src, ctx) {
				ctx = mergeObjects(
					makeContext(document.body, null, document.body, null),
					ctx || {}
				);
				...
				if (elt.querySelectorAll) {
					forEach(
						elt.querySelectorAll(
							selector + ", [type='text/hyperscript']"
						),
						function (elt) {
							initElement(
								elt,
			...
			function initElement(elt, target) {
				if (
					elt.closest &&
					elt.closest(_hyperscript.config.disableSelector)
				) {
					return;
				}
				...
				if (root != null) {
					...
					if (isArrayLike(root)) {
						...
						for (var i = 0; i < root.length; i++) {
							...
							var componentValue = attribute
								? component.getAttribute(property)
								: component[property];
							if (componentValue) {
					...
					var traceEntry = {
						...
						print: function (logger) {
							...
							for (var i = 0; i < trace.length; i++) {
								...
								logger(
									"  ->",
									traceElt.meta.feature.displayName.padEnd(
										maxLen + 2
									),
									"-",
									traceElt.meta.owner
			...
			/** @type string | null */
			var hyperscriptUrl =
				"document" in globalScope ? document.currentScript.src : null;
		...
		//====================================================================
		{
						...
						return {
							...
							evaluate: function (context) {
								return context.me
									.getRootNode()
									.querySelectorAll(
										runtime.escapeSelector(this.css)
									);
							},
						};
					}
				...
				function (parser, runtime, tokens) {
					...
					if (attributeRef) {
						...
						if (outerVal.indexOf("[") === 0) {
							var innerValue = outerVal.substring(
								2,
								outerVal.length - 1
							);
						} else {
			...
			_parser.addGrammarElement(
				...
				function (parser, runtime, tokens) {
					...
					if ((token = tokens.matchTokenType("STRING"))) {
						...
						do {
							token =
								tokens.matchTokenType("IDENTIFIER") ||
								tokens.matchOpToken("-");
			...
			_parser.addLeafExpression(
				...
				function (parser, runtime, tokens) {
					if (tokens.matchOpToken("{")) {
						...
						if (!tokens.matchOpToken("}")) {
							do {
								var name = parser.requireElement(
									"objectKey",
									tokens
								);
								tokens.requireOpToken(":");
								var value = parser.requireElement(
									"expression",
									tokens
								);
				...
				function (parser, runtime, tokens) {
					if (tokens.matchOpToken("(")) {
						...
						if (!tokens.matchOpToken(")")) {
							do {
								var name =
									tokens.requireTokenType("IDENTIFIER");
								tokens.requireOpToken(":");
								var value = parser.requireElement(
									"expression",
									tokens
					...
					if (tokens.matchOpToken(".")) {
						...
						var propertyAccess = {
							...
							op: function (_context, rootVal) {
								var value = runtime.resolveProperty(
									rootVal,
									prop.value
								);
								return value;
			...
			_parser.addIndirectExpression(
				...
				function (parser, runtime, tokens, root) {
					if (tokens.matchToken("of")) {
						var newRoot = parser.requireElement(
							"expression",
							tokens
						);
						// find the urroot
						...
						if (
							...
							parser.raiseParseError(
								tokens,
								"Cannot take a property of a non-symbol: " +
									urRoot.type
							);
						}
						...
						var propertyAccess = {
							...
							op: function (context, rootVal) {
								return runtime.resolveProperty(
									rootVal,
									prop,
									attribute
								);
			...
			_parser.addIndirectExpression(
				...
				function (parser, runtime, tokens, root) {
					...
					if (
						...
						var attribute = parser.parseElement(
							"attributeRef",
							tokens
						);
						if (attribute == null) {
			...
			_parser.addIndirectExpression(
				...
				function (parser, runtime, tokens, root) {
					if (tokens.matchToken("in")) {
						...
						var target = parser.requireElement(
							"expression",
							tokens
						);
						var propertyAccess = {
			...
			_parser.addIndirectExpression(
				...
				function (parser, runtime, tokens, root) {
					if (tokens.matchOpToken("[")) {
						...
						if (tokens.matchOpToken("..")) {
							andBefore = true;
							firstIndex = parser.requireElement(
								"expression",
								tokens
							);
						} else {
							...
							if (tokens.matchOpToken("..")) {
								...
								if (current.type !== "R_BRACKET") {
									secondIndex = parser.parseElement(
										"expression",
										tokens
									);
								}
								...
								if (andBefore) {
									...
									if (secondIndex != null) {
										return root.slice(
											firstIndex,
											secondIndex + 1
										); // returns all items from firstIndex to secondIndex (inclusive)
									} else {
			...
			_parser.addGrammarElement(
				...
				function (parser, runtime, tokens) {
					if (tokens.matchToken("not")) {
						var root = parser.requireElement(
							"unaryExpression",
							tokens
						);
						return {
			...
			_parser.addGrammarElement(
				...
				function (parser, runtime, tokens) {
					...
					var comparisonStr = comparisonToken
					...
					if (comparisonStr == null) {
						if (
							...
							tokens.matchToken("do") ||
							tokens.matchToken("does")
						) {
						...
						expr = {
							...
							op: function (context, lhsVal, rhsVal) {
								...
								if (this.operator === "in") {
									return (
										rhsVal != null &&
										Array.from(rhsVal).indexOf(lhsVal) >= 0
									);
								}
								...
								if (this.operator === "match") {
									return (
										lhsVal != null && lhsVal.matches(rhsVal)
									);
								}
								if (this.operator === "not match") {
									return (
										lhsVal == null ||
										!lhsVal.matches(rhsVal)
									);
								}
								if (this.operator === "contain") {
									return (
										lhsVal != null &&
										lhsVal.contains(rhsVal)
									);
								}
								...
								if (this.operator === "===") {
									...
									throw (
										"Unknown comparison : " + this.operator
									);
								}
							},
							evaluate: function (context) {
			...
			_parser.addGrammarElement(
				...
				function (parser, runtime, tokens) {
					...
					logicalOp =
						tokens.matchToken("and") || tokens.matchToken("or");
					while (logicalOp) {
			...
			_parser.addGrammarElement(
				...
				function (parser, runtime, tokens) {
					return parser.parseAnyOf(
						["logicalOperator", "mathExpression"],
						tokens
					);
				}
			);
				...
				function (parser, runtime, tokens) {
					...
					if (tokens.hasMore()) {
						while (
							...
							var feature = parser.requireElement(
								"feature",
								tokens
							);
			...
			_parser.addFeature("on", function (parser, runtime, tokens) {
				if (tokens.matchToken("on")) {
					...
					do {
						...
						if (eventName === "intersection") {
							...
							if (tokens.matchToken("with")) {
								intersectionSpec["with"] = parser
									.parseElement("targetExpression", tokens)
									.evaluate();
							}
							...
							if (tokens.matchToken("of")) {
								do {
									if (tokens.matchToken("anything")) {
										...
										tokens.matchToken("characterData")
									) {
										mutationSpec["characterData"] = true;
										mutationSpec[
						...
						if (tokens.matchToken("in")) {
							var inExpr = parser.parseAnyOf(
								["idRef", "queryRef", "classRef"],
								tokens
							);
						}
						...
						if (tokens.matchToken("debounced")) {
							...
							var timeExpr = parser.requireElement(
								"timeExpression",
								tokens
							);
					...
					var onFeature = {
						...
						execute: function (/** @type {Context} */ ctx) {
							...
							ctx.meta.reject = function (err) {
								...
								var hypertrace = runtime.getHyperTrace(
									ctx,
									err
								);
								if (hypertrace) {
			...
			_parser.addFeature("def", function (parser, runtime, tokens) {
				if (tokens.matchToken("def")) {
					var functionName = parser.requireElement(
						"dotOrColonPath",
						tokens
					);
					...
					if (tokens.matchToken("catch")) {
						var errorSymbol =
							tokens.requireTokenType("IDENTIFIER").value;
						var errorHandler = parser.parseElement(
							"commandList",
							tokens
						);
					}
					var functionFeature = {
						...
						install: function (target, source) {
							var func = function () {
								...
								if (ctx.meta.caller) {
									ctx.meta.callingCommand =
										ctx.meta.caller.meta.command;
								}
								var resolve,
									reject = null;
			...
			_parser.addFeature("behavior", function (parser, runtime, tokens) {
				...
				var path = parser
					.parseElement("dotOrColonPath", tokens)
					.evaluate();
						...
						runtime.unifiedEval(
							{
								...
								op: function (ctx, args) {
									...
									for (
										var i = 0;
										i < behaviorNamespace.length;
										i++
									) {
										behavior =
											behavior[behaviorNamespace[i]];
										if (
											typeof behavior !== "object" &&
											typeof behavior !== "function"
										)
											throw new Error(
												"No such behavior defined as " +
													behaviorPath
											);
									}
			...
			_parser.addGrammarElement(
				...
				function (parser, runtime, tokens) {
					...
					return {
						...
						jsSource: tokens.source.substring(
							jsSourceStart,
							jsSourceEnd
						),
					};
			...
			_parser.addCommand("js", function (parser, runtime, tokens) {
				if (tokens.matchToken("js")) {
					...
					var func = varargConstructor(
						Function,
						inputs.concat([jsBody.jsSource])
					);
					...
					return {
						...
						op: function (context) {
							...
							inputs.forEach(function (input) {
								args.push(
									runtime.resolveSymbol(input, context)
								);
							});
				...
				if (tokens.matchToken("wait")) {
					...
					if (tokens.matchToken("for")) {
						...
						if (tokens.matchToken("a")) {
							...
							var time = _parser.requireElement(
								"timeExpression",
								tokens
							);
						}
						...
						var waitCmd = {
							...
							op: function (context, timeValue) {
								return new Promise(function (resolve) {
									setTimeout(function () {
										resolve(
											runtime.findNext(waitCmd, context)
										);
									}, timeValue);
								});
							},
			...
			_parser.addCommand("send", function (parser, runtime, tokens) {
				if (tokens.matchToken("send")) {
					var eventName = parser.requireElement(
						"dotOrColonPath",
						tokens
					);

					var details = parser.parseElement(
						"namedArgumentList",
						tokens
					);
					if (tokens.matchToken("to")) {
						var to = parser.requireElement(
							"targetExpression",
							tokens
						);
					} else {
			...
			var parseReturnFunction = function (
				parser,
				runtime,
				tokens,
				returnAValue
			...
			_parser.addCommand("log", function (parser, runtime, tokens) {
				if (tokens.matchToken("log")) {
					...
					if (tokens.matchToken("with")) {
						var withExpr = parser.requireElement(
							"expression",
							tokens
						);
					}
			...
			_parser.addGrammarElement(
				...
				function (parser, runtime, tokens) {
					...
					if (
						...
						expr.root.type !== "symbol"
					) {
						parser.raiseParseError(
							"Implicit function calls must start with a simple function",
							tokens
						);
					}
					// optional "on", "with", or "to"
					if (
						!tokens.matchAnyToken("to", "on", "with") &&
			...
			_parser.addCommand("if", function (parser, runtime, tokens) {
				if (tokens.matchToken("if")) {
					...
					if (tokens.matchToken("else")) {
						var falseBranch = parser.parseElement(
							"commandList",
							tokens
						);
					}
			...
			var parseRepeatExpression = function (
				...
				if (tokens.matchToken("for") || startedWithForToken) {
					...
					var expression = parser.requireElement(
						"expression",
						tokens
					);
				} else if (tokens.matchToken("in")) {
					...
					if (tokens.matchToken("event")) {
						var evt = _parser.requireElement(
							"dotOrColonPath",
							tokens,
							"Expected event name"
						);
						...
						var whileExpr = parser.requireElement(
							"expression",
							tokens
						);
					}
				...
				var repeatCmd = {
					...
					op: function (context, whileValue) {
						...
						if (this.forever) {
							...
							if (evt) {
								keepLooping =
									context.meta.iterators[slot].eventFired ===
									false;
							} else {
			...
			_parser.addCommand("increment", function (parser, runtime, tokens) {
				if (tokens.matchToken("increment")) {
					...
					var target = parser.parseElement(
						"targetExpression",
						tokens
					);
		...
		function mergeMetaConfig() {
			...
			if (metaConfig) {
				_hyperscript.config = mergeObjects(
					_hyperscript.config,
					metaConfig
				);
			}
		...
		return mergeObjects(
			...
			},
			{
				...
				config: {
					...
					disableSelector:
						"[disable-scripting], [data-disable-scripting]",
